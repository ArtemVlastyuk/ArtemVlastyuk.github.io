...
button.addEventListener("submit",function (event){
    ...
    if (!error) {
        ...
        //alert(JSON.parse(localStorage.getItem("myKey")));
    }
});
...
button_log.addEventListener("submit",function (event){
    ...
    for (let i=0;i<log.length;i++){
        ...
        if(email===log[i].email){
            ...
            if(password==log[i].password){
                ...
                //setTimeout(function() { button_log.submit},10000 },
                check=true;
                //alert("Добро пожаловать, "+log[i].name+"!");
                //autorization=true;

                localStorage.setItem("info",JSON.stringify(log[i]));
                localStorage.setItem("autorization", JSON.stringify(true));
    ...
    if(!check) {
        ...
        newElement.innerHTML = '<p>Неверный логин!</p>';
        var timeout_id = setTimeout(function(){
            footer.after(newElement);
        }, 100);
...
if(log==null || log.length==0){
    ...
    localStorage.setItem("myKey",JSON.stringify(login));
